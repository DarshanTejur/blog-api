...
@router.post(
    "/new-blog", status_code=status.HTTP_201_CREATED, response_model=schemas.BlogOut
)
def create_new_blog(
...
def get_blog(
    ...
    if not blog:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"{id}, blog not found"
        )
    return blog


@router.get("/get-blogs-current-user", status_code=status.HTTP_200_OK)
def get_blog_ownerid(
    db: Session = Depends(database.get_db),
    current_user: int = Depends(oauth2.get_current_user),
):
    ...
    if not blogs:
        raise HTTPException(
            ...
            detail=f"{current_user.id},no blog not found",
        )
    return blogs


@router.get("/get-blogs-users/{id}", status_code=status.HTTP_200_OK)
def get_blog_user_ownerid(
    id: int,
    db: Session = Depends(database.get_db),
    current_user: int = Depends(oauth2.get_current_user),
):
    ...
    if not blogs:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"{id},users blog not found"
        )
    return blogs
...
def delete_blog(
    ...
    if blog_delete is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"{id}, record not found"
        )
